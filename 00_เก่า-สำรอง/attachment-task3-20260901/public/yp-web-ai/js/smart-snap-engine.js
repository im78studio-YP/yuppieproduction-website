(function(global){
  ...
  const ANCHOR_TYPES=Object.freeze(['bottom','back','front','left','right','top','center','mount']);
  ...
  class SmartSnapEngine{
    ...
    validate(){const errors=[],anchorIds=new Set(),surfaceIds=new Set();this.listAnchors().forEach(anchor=>{
      ...
      return{valid:errors.length===0,errors,stats:{anchors:anchorIds.size,surfaces:surfaceIds.size}};}
  }
  ...
  global.YPSmartSnap=Object.freeze({SMART_SNAP_VERSION,GRID_STEP,SURFACE_DETECTION_DISTANCE,SCREEN_THRESHOLD,SNAP_RELEASE_DISTANCE,SNAP_RELEASE_SCREEN_THRESHOLD,
    ...
    createAnchorsForAsset,createSurfacesForAsset,worldSurface,quantizeSurfacePoint,rotateVector});
