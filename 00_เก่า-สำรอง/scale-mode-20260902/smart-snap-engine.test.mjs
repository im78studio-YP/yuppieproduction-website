...
test('Logo Alpha Bounds ตัดพื้นที่โปร่งใสและรักษาอัตราส่วน Artwork จริง',()=>{
  ...
  const bounds=brandBoundsApi.scanAlphaBounds(data,width,height),metrics=brandBoundsApi.planeMetrics({width:6,height:3,depth:.05,bounds,padding:.03});
  assert.deepEqual(plain(bounds),{minX:.3,maxX:.9,minY:.25,maxY:.75});
  assert.ok(Math.abs(metrics.contentWidth-3.6)<1e-9);assert.ok(Math.abs(metrics.contentHeight-1.5)<1e-9);
  assert.ok(Math.abs(metrics.localX-.6)<1e-9);assert.equal(metrics.localY,0);
  assert.ok(Math.abs(metrics.width-3.66)<1e-9);assert.ok(Math.abs(metrics.height-1.56)<1e-9);
});

test('Logo Corner Snap ใช้ Bounds ที่ครอปแล้วครบ 8 มุม ไม่ใช้ขนาด Plane เดิม',()=>{
  const metrics=brandBoundsApi.planeMetrics({width:6,height:3,depth:.05,bounds:{minX:.35,maxX:.65,minY:.35,maxY:.65},padding:.03}),
    record=asset('branding.logo.main','Logo','branding',{width:metrics.width,height:metrics.height,depth:metrics.depth},{x:3,y:1.6,z:.08},{system:true,movable:false}),
    corners=snapApi.createAnchorsForAsset(record).filter(anchor=>anchor.anchorType==='corner');
  assert.equal(corners.length,8);assert.ok(metrics.width<2);assert.ok(metrics.height<1);
  assert.deepEqual([...new Set(corners.map(anchor=>anchor.localPosition.x))].sort((a,b)=>a-b),[-metrics.width/2,metrics.width/2]);
  assert.deepEqual([...new Set(corners.map(anchor=>anchor.localPosition.y))].sort((a,b)=>a-b),[-metrics.height/2,metrics.height/2]);
});
...
test('Resize Handle แยกจาก Snap Point ใช้ 8 มุม Fixed Pivot และ Commit เป็น History เดียว',()=>{
  for(const token of ['id="btnResizeObject"','resizeMode:false','buildResizeHandles(id)','object-resize-handles','objectLocalBounds(root)',
    ...
    "kind:'resize'",'moveResizeDrag(event,drag)','finishResizeDrag(event,drag)','cancelActiveResizeDrag()','recordObjectHistory(drag.before)'])assert.ok(html.includes(token),token);
  ...
  assert.match(html,/if\(primaryId&&objectById\(primaryId\)&&objectEditor\.resizeMode\)this\.buildResizeHandles/);
});

test('Logo Selection, Registry และ Geometry ใช้ Visible Bounds ชุดเดียวกัน',()=>{
  for(const token of ['js/brand-visible-bounds.js','logoVisibleBounds:null','brandVisibleAssetLayout(spec)','sceneSelectionTargets(id)',
    'part.userData.applyBrandVisibleBounds(visible)','spec.logoVisibleBounds={...visible}','screenBrandAnchorSnap(event,sourceAnchorId)'])assert.ok(html.includes(token),token);
  assert.match(html,/record\.id===SCENE_ASSET_IDS\.brand&&renderer\.brandingHitTarget/);
  assert.match(html,/id===SCENE_ASSET_IDS\.brand&&this\.brandingHitTarget/);
  assert.match(html,/if\(primaryId&&\(objectById\(primaryId\)\|\|primaryId===SCENE_ASSET_IDS\.brand\)\)/);
  assert.match(html,/recordObjectHistory\(drag\.before\);this\.currentKey='';sync\(\)/);
});
