  ...
  class AIRenderPipeline {
    ...
    async execute(input) {
      ...
      try {
        ...
        const reference = await measure('prepareReferenceImage', () => requiresReference
          ? (input.referenceImage || input.prepareReferenceImage(context))
          : null);
        const referenceMeta = requiresReference ? validateReferenceImage(reference, input.referenceLimits) : null;
