(function initAIRenderIntent(global) {
  ...
  function buildConceptPrompt(context = {}) {
    ...
    const openSides = text(booth.openSides, 'ตามมาตรฐานของรูปแบบบูธ');
    const corner = text(booth.cornerSide);
    const quality = normalizeQuality(context.quality);
    const qualityLine = quality === QUALITY.FINAL
      ? 'คุณภาพภาพ: Final · Photorealistic รายละเอียดสูง แสง วัสดุ และผิวงานพร้อมสำหรับภาพนำเสนอ'
      : 'คุณภาพภาพ: Preview · สร้างแนวคิดรวดเร็ว รายละเอียดปานกลาง เหมาะสำหรับตรวจทิศทางการออกแบบ';
    return [
      'AI RENDER INTENT — CONCEPT DESIGN',
      'สร้างภาพแนวคิดบูธที่เปิดใช้งานจริงจากข้อมูล Project State โดยไม่ใช้ภาพอ้างอิงจากระบบ 3D',
      ...
      `ด้านเปิด: ${openSides}${corner ? ` · หัวมุม${corner === 'left' ? 'ซ้าย' : corner === 'right' ? 'ขวา' : corner}` : ''}`,
      `ข้อกำหนดตั้งต้น: ${floor} · ${storage}`,
      `สไตล์: ${style}`,
      qualityLine,
      '',
      'GEOMETRY FIDELITY — CONCEPT',
      'รักษาประเภทธุรกิจ สีแบรนด์ สไตล์ รูปแบบบูธ ด้านเปิด หัวมุม และขนาดรวมที่กำหนด',
      'ให้อิสระออกแบบการแบ่งโซน เคาน์เตอร์ ชั้นและแท่นแสดงสินค้า โต๊ะและเก้าอี้ สินค้า Generic คน ต้นไม้ ของตกแต่ง แสง และบรรยากาศได้อย่างสร้างสรรค์',
