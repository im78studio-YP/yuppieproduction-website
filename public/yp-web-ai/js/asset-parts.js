(function(global){
 ...
 global.YPAssetParts={sanitize,capture,seal,apply,entries};
 ...
 async function open(){
  ...
  try{
   ...
   if(!document.getElementById('mAssetSettings').classList.contains('show')||objectEditor.selectedId!==object.id)return;
   if(!parts.length||parts.some(e=>!e.inherited))throw Error('อุปกรณ์นี้ยังไม่รองรับการแก้ชิ้นส่วน หรือพื้นผิวยังโหลดไม่ครบ');
   launch(renderer,source,parts,object,trigger);
 ...
 function launch(r,source,sourceParts,object,trigger){
  ...
  $('#assetPartsTitle').textContent='แก้ชิ้นส่วน · '+selectedAssetName(object);
  const baseline=JSON.stringify(object.appearance),draft=sanitize(object.appearance.parts);
  document.body.append(dialog);dialog.showModal();
  let gl,controls,observer,highlight,environment,pathEditor,selected=0,frameId=0,closed=false,down=null;
  ...
  const clonedEffects=[];model.traverse(node=>{if(node.userData.partEdgeGlow)clonedEffects.push(node);});clonedEffects.forEach(node=>node.removeFromParent());
  ...
  $('[data-part-info]').textContent=meshCount===1?'โมเดลมี Mesh เดียว'+(parts.length>1?' แต่เลือกกลุ่มวัสดุได้ '+parts.length+' กลุ่ม':' จึงแก้ได้ทั้งก้อน ยังแยกท็อปหรือแผงหน้าไม่ได้'):meshCount+' ชิ้น / '+parts.length+' กลุ่มวัสดุ · ชื่อตามไฟล์โมเดล';
  ...
  function dispose(){
   ...
   pathEditor?.dispose();if(highlight){highlight.geometry.dispose();highlight.material.dispose();}
  ...
  try{gl=new T.WebGLRenderer({antialias:true,alpha:false});gl.setPixelRatio(Math.min(devicePixelRatio,2));gl.outputColorSpace=T.SRGBColorSpace;gl.toneMapping=T.ACESFilmicToneMapping;viewport.append(gl.domElement);controls=new r.controls.constructor(camera,gl.domElement);controls.enableDamping=true;
   ...
   function update(){
    ...
    finish.disabled=value==='inherit'||value==='original';status.textContent='ภาพทดลอง · ยังไม่บันทึกลงแบบหลัก';
   }
  ...
  $('[data-part-save]').onclick=()=>{
   ...
   if(current!==object||objectLocked(current)||JSON.stringify(current.appearance)!==baseline){status.textContent='อุปกรณ์ถูกเปลี่ยนหรือล็อก กรุณายกเลิกแล้วเปิดใหม่';return;}
   const before=objectSnapshot();current.appearance.parts=sanitize(draft);recordObjectHistory(before);dispose();sync();syncAssetSettingsForm();announceCatalog('บันทึกพื้นผิวเฉพาะชิ้นส่วนแล้ว · Undo เพื่อย้อนกลับ','success');
  };
 }
 global.YPAssetParts.open=open;
 const button=document.createElement('button');button.id='assetEditParts';button.type='button';button.className='btn asset-settings-done';button.textContent='แก้ไขชิ้นส่วน · เปิดภาพ 3D';button.onclick=open;
