(function(){
  ...
  function artwork(t){
    ...
    if(t.reference)return canvas.toDataURL('image/png');
    ...
    if(t.id==='technology'){c.strokeStyle=t.secondary;c.lineWidth=4;for(let x=60;x<1800;x+=100){c.beginPath();c.moveTo(x,0);c.lineTo(x+250,720);c.stroke();}c.fillStyle=t.background;c.fillRect(230,100,1050,460);c.strokeRect(230,100,1050,460);c.fillStyle='#74d8cf';c.font='bold 34px sans-serif';}
    ...
    c.fillStyle=t.ink;c.font='500 24px sans-serif';c.textAlign='center';c.globalAlpha=.6;c.font='18px sans-serif';
  ...
  function snapshot(id,options={}){
    ...
    original.objects.forEach((part,i)=>{if(!part.graphic)return;
      const canvas=document.createElement('canvas');canvas.width=1000;canvas.height=500;const c=canvas.getContext('2d');
      c.fillStyle=part.graphic==='screen'?'#078dcc':part.graphic==='counter'?'#b99a72':'#ffffff';c.fillRect(0,0,1000,500);
      if(part.graphic==='screen'){c.fillStyle='#125b94';for(const points of [[[80,95],[260,70],[355,170],[250,210],[200,320],[120,230]],[[430,100],[670,65],[860,180],[720,290],[555,200]],[[690,325],[840,345],[870,410],[740,430]]]){c.beginPath();points.forEach(([x,y],i)=>i?c.lineTo(x,y):c.moveTo(x,y));c.closePath();c.fill();}}
      c.fillStyle=part.graphic==='screen'||part.graphic==='counter'?'#ffffff':t.primary;c.textAlign='center';c.font='italic 64px sans-serif';
      if(!['screen','counter'].includes(part.graphic)){c.font='25px sans-serif';c.fillStyle='#777777';}
      if(part.graphic==='header'){
        canvas.height=320;c.fillStyle='#f5f3ed';c.fillRect(0,0,1000,320);c.fillStyle=t.primary;c.textAlign='center';c.font='italic 100px sans-serif';c.fillStyle='#94948e';c.font='26px sans-serif';
      }
    ...
    YPTemplateBranding.apply(result.spec,t);window.YPInlineSoftWave?.decorate(result.spec,t);YPTVAsset.template(result.spec,t);YPProjectStore.validateSpec(result.spec);return result;
  }
  ...
  entry.innerHTML='<h3>ตัวอย่างบูธ Inline 3×6 ม.</h3><p>'+library.templates.length+' ผังพร้อมวัสดุและกราฟิกตัวอย่าง แก้ไขทุกชิ้นต่อได้</p><button class="btn pri" id="inlineTemplatesOpen" type="button">เลือกเทมเพลต '+library.templates.length+' แบบ</button><small>หน้ากว้าง 6 ม. × ลึก 3 ม. · เปิดด้านหน้า</small>';
  ...
  library.templates.forEach(t=>{
    ...
    const img=document.createElement('img');img.src='assets/inline-templates/'+t.id+'.png?v=20260911-brand-audit';img.alt='ภาพโมเดลจริง '+t.name;img.width=960;img.height=720;img.loading='lazy';
