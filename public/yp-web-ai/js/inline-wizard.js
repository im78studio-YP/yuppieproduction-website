(function(){
  ...
  const libraryFor=type=>type==='inline'?window.YPInlineTemplates:type==='penin'?window.YPPeninsularTemplates:null;
  const bridgeFor=type=>type==='inline'?window.YPInlineTemplateBridge:window.YPPeninsularTemplateBridge;
  const imageFor=(type,id)=>'assets/'+(type==='inline'?'inline':'peninsular')+'-templates/'+id+'.png';
  ...
  host.querySelector('.quick-template-heading span').textContent=(type==='inline'?'Inline':'Peninsular')+' · เทมเพลตกว้าง 6 × ลึก 3 ม.';
  }
  ...
  async function complete(draft,initialWizard){
    ...
    return YPProjectWorkspace.useTemplate({initialWizard,name:t.name,makeSnapshot:()=>{
      const result=bridgeFor(draft.boothType).snapshot(t.id,{primary:draft.primary}),s=result.spec;
