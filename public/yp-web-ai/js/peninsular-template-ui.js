(function(){
  ...
  function referenceArtwork(kind){
    ...
    if(kind==='blue-info'){c.fillStyle='#f4f4ec';c.fillRect(0,0,1000,650);c.fillStyle='#25658f';c.font='bold 54px sans-serif';c.textAlign='center';for(let i=0;i<3;i++){c.fillStyle='#b5c7c9';c.fillRect(125,200+i*35,750-i*70,9);c.fillStyle='#078caf';c.beginPath();c.arc(210+i*290,435,61,0,Math.PI*2);c.fill();c.fillStyle='#ffffff';c.font='bold 55px sans-serif';}c.fillStyle='#25658f';c.font='20px sans-serif';}
  ...
  function snapshot(id,options={}){const t={...library.templates.find(t=>t.id===id)},result=library.build(id,JSON.parse(INITIAL_BOOTH_SPEC_JSON),OBJECT_CATALOG);
    ...
    result.spec.wallStickerFaces=t.customBack?[]:['back'];Object.assign(result.spec.wallStickers.back,{data:artwork(t),name:t.name+' · กราฟิกตัวอย่าง',id:1,ar:2.5,w:6,h:2.4,mode:'cover'});YPTemplateBranding.apply(result.spec,t);YPTVAsset.template(result.spec,t);YPProjectStore.validateSpec(result.spec);return result;}
  window.YPPeninsularTemplateBridge={snapshot};if(new URLSearchParams(location.search).get('comparePreview')==='1')return;
  const entry=document.createElement('section');entry.id='peninsularTemplateSubmenu';entry.className='starter-entry';entry.innerHTML='<h3>เทมเพลต Peninsular 3×6 และ 6×6 ม.</h3><p>'+library.templates.length+' แบบตามภาพอ้างอิง · เปิดหน้า ซ้าย และขวา</p><button type="button" class="btn pri" id="peninsularTemplatesOpen">เลือกเทมเพลต '+library.templates.length+' แบบ</button>';
