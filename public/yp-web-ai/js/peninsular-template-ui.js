(function(){
  ...
  function artwork(t){const canvas=document.createElement('canvas');canvas.width=1800;canvas.height=720;const c=canvas.getContext('2d');c.fillStyle=t.background;c.fillRect(0,0,1800,720);
    if(t.id==='penin-blue-step')return canvas.toDataURL('image/png');
  ...
  function panelArtwork(kind){if(['red-line','sage-rings','blush-beauty','blue-info'].includes(kind))return referenceArtwork(kind);const canvas=document.createElement('canvas');canvas.width=900;canvas.height=600;const c=canvas.getContext('2d'),dark=kind==='blue-rings',bg=c.createRadialGradient(450,300,20,450,300,580);bg.addColorStop(0,dark?'#135eac':'#ffffff');bg.addColorStop(1,dark?'#031128':'#e6f2f6');c.fillStyle=bg;c.fillRect(0,0,900,600);for(let i=0;i<12;i++){c.strokeStyle=dark?(i%2?'#2376de':'#5bd7ff'):(i%2?'#bed6e3':'#58badc');c.lineWidth=i%3===0?5:2;c.beginPath();c.ellipse(450,300,75+i*26,70+i*19,0,-Math.PI*.91,Math.PI*.86);c.stroke();}c.fillStyle=dark?'#ffffff':'#487890';c.textAlign='center';c.font='22px sans-serif';c.fillText('YUPPIE / CONNECTIONS',450,560);return canvas.toDataURL('image/png');}
