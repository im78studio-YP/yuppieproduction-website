...
test('SnapAnchor และ SnapSurface schema/enums ตรงตาม Task 2',()=>{
  assert.deepEqual(Array.from(snapApi.ANCHOR_TYPES),['bottom','back','front','left','right','top','center','mount']);
...
test('Priority และ Validation ไม่มี ID ซ้ำ',()=>{
  ...
  const report=engine.validate();assert.equal(report.valid,true);assert.ok(report.stats.anchors>0);assert.ok(report.stats.surfaces>0);
});
