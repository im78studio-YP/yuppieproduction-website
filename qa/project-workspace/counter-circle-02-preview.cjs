...
(async()=>{await fs.mkdir(out,{recursive:true});const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 const items=await page.evaluate(async()=>{
  ...
  for(const item of OBJECT_CATALOG.filter(i=>i.catalogId==='imported-counter-circle-02'&&i.modelUrl))r.requestFurnitureTemplate(item);

  await Promise.all([...r.furnitureLoads.values(),...r.fixtureLoads.values()]);
  for(const item of OBJECT_CATALOG.filter(i=>i.catalogId==='imported-counter-circle-02'&&i.modelUrl))if(!r.furnitureTemplates.has(item.catalogId))throw Error('Missing model: '+item.catalogId);
  return OBJECT_CATALOG.filter(i=>i.catalogId==='imported-counter-circle-02').map(i=>({id:i.catalogId,name:i.name}));
 });
 for(const item of items){const data=await page.evaluate(async id=>{
  ...
  const sourceGLB=await r.modelLoader.loadAsync(objectCatalogDef(id).modelUrl),sourceSize=new T.Box3().setFromObject(sourceGLB.scene).getSize(new T.Vector3());[sourceSize.x,sourceSize.y,sourceSize.z].forEach((v,i)=>{if(Math.abs(v-[2.798,.9,3][i])>.001)throw Error('Source dimensions mismatch');});console.log('Source bounds',sourceSize.toArray());
  if(bounds.isEmpty())throw Error('Empty geometry: '+id);
  root.position.sub(center);root.updateMatrixWorld(true);
  [size.x,size.y,size.z].forEach((v,i)=>{if(Math.abs(v-[2.798,.9,3][i])>.005)throw Error('Wrong counter dimensions');});if(YPAssetParts.entries(root).length!==1)throw Error('Expected 1 source mesh');
