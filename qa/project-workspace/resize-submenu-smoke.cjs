...
(async()=>{
 ...
 try{for(const mobile of [false,true]){
  ...
  await page.route('https://fonts.googleapis.com/**',r=>r.abort());await page.goto('http://127.0.0.1:4173/yp-web-ai/index.html',{waitUntil:'domcontentloaded'});
  ...
  await page.locator('#resizeValueh').fill('31');await page.locator('#resizeValueh').press('Enter');assert.equal(await page.locator('#resizeValueh').getAttribute('aria-invalid'),'true');assert.deepEqual(await size(),{w:1,d:.2,h:2});
  await page.locator('#resizeValued').fill('4');await page.locator('#resizeValued').press('Enter');assert.match(await page.locator('#resizeSubmenuStatus').textContent(),/ทุกด้าน/);assert.deepEqual(await size(),{w:1,d:.2,h:2});
  ...
  await page.locator('#resizeSubmenuMode').selectOption('scale');assert.equal(await page.evaluate(()=>objectEditor.transformMode),'scale');assert.equal(await panel.isVisible(),true);assert.equal(await page.locator('#resizeSubmenuLock').isDisabled(),true);
  await page.locator('#resizeValuew').fill('2');await page.locator('#resizeValuew').press('Enter');assert.deepEqual(await size(),{w:1,d:.3,h:1.5});assert.equal(await page.evaluate(id=>sceneObjectScaleValue(objectById(id)),id),2);assert.equal(await page.locator('#resizeValueh').inputValue(),'3');
  await page.evaluate(()=>undoObjectChange());assert.equal(await page.evaluate(id=>sceneObjectScaleValue(objectById(id)),id),1);
  await page.locator('#resizeSubmenuMode').selectOption('resize');assert.equal(await page.evaluate(()=>objectEditor.transformMode),'resize');assert.equal(await page.locator('#resizeSubmenuLock').isDisabled(),false);assert.equal(await page.locator('#resizeValueh').inputValue(),'1.5');
