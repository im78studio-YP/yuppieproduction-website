...
(async()=>{
 ...
 try{for(const mobile of [false,true]){
  ...
  await page.locator('#resizeSubmenuLock').check();await page.locator('#resizeValuew').fill('1');await page.locator('#resizeValuew').press('Enter');assert.deepEqual(await size(),{w:1,d:.2,h:2});
  ...
  await page.locator('[data-resize-reset="h"]').click();assert.equal(await page.evaluate(id=>sceneObjectScaleValue(objectById(id)),id),1);
  await page.evaluate(()=>undoObjectChange());assert.equal(await page.evaluate(id=>sceneObjectScaleValue(objectById(id)),id),2);
  await page.locator('[data-resize-reset="w"]').click();assert.equal(await page.evaluate(id=>sceneObjectScaleValue(objectById(id)),id),1);
  ...
  assert.equal(await page.locator('[data-resize-reset="w"]').textContent(),'↶');
  const beforeReset=await size(),originalWidth=await page.evaluate(id=>objectById(id).originalSize.w,id);
  await page.locator('#resizeValuew').fill('9');await page.locator('[data-resize-reset="w"]').click();
  assert.deepEqual(await size(),{...beforeReset,w:originalWidth});
  await page.evaluate(()=>undoObjectChange());assert.deepEqual(await size(),beforeReset);
