...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true,args:['--enable-unsafe-swiftshader']});try{
 ...
 await page.evaluate(()=>YPPeninsularTemplateUI.open());assert.equal(await page.locator('[data-penin-template="penin-oak-gallery-6"]').count(),1);
 assert.deepEqual(errors,[]);console.log('PASS 3 views, real counter opening, TV artwork, template picker and Wizard without modifying active design');
