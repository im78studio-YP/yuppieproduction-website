...
async function writeAsset(path,data){const tmp=path+'.tmp',bytes=Buffer.isBuffer(data)?data:Buffer.from(data);await fs.writeFile(tmp,bytes);const hash=b=>crypto.createHash('md5').update(b).digest('hex');assert.equal(hash(await fs.readFile(tmp)),hash(bytes));for(let attempt=0;;attempt++){try{await fs.rename(tmp,path);break;}catch(e){if(!['EPERM','EBUSY'].includes(e.code)||attempt>=12)throw e;await new Promise(resolve=>setTimeout(resolve,250));}}}
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
  ...
  const templates=await page.evaluate(()=>YPCornerTemplates.templates.map(t=>t.id));assert.equal(templates.length,6);
  for(const id of templates)for(const side of ['right','left']){
    ...
    await writeAsset('public/yp-web-ai/assets/corner-templates/'+id+'-'+side+'.png',Buffer.from(result.png,'base64'));console.log('RENDERED',id,side);
  }
  await page.goto(base+'index.html');await page.waitForFunction(()=>window.YPInlineWizard&&YPProjectWorkspace.state().ready);await page.locator('#releaseStart').click();await page.locator('#quickBusinessNext').click();await page.locator('#quickBoothTypes [data-value=corner]').click();assert.equal(await page.locator('#quickTemplateChoices [role=radio]').count(),7);
  ...
  await page.evaluate(()=>YPCornerTemplateUI.open());await page.locator('#cornerTemplateSide').selectOption('left');assert.ok((await page.locator('#cornerCards img').first().getAttribute('src')).endsWith('-left.png'));await page.locator('[data-corner-template]').first().click();await page.waitForFunction(()=>!YPProjectWorkspace.state().busy&&YPProjectWorkspace.state().active==='B');assert.equal(await page.evaluate(()=>getBoothSpec().cornerSide),'left');assert.equal(await page.locator('#cornerCards a').count(),0);
  assert.deepEqual(errors,[]);
  for(const id of templates)for(const side of ['left','right']){const ctx=await browser.newContext(),p=await ctx.newPage();await p.goto(base+'assets/corner-templates/overview.html');assert.equal(await p.locator('.use-template').count(),12);assert.equal(await p.locator('[download]').count(),0);await p.locator('.use-template[href*="useTemplate='+id+'&"][href$="cornerSide='+side+'"]').click();await p.waitForFunction(()=>window.YPProjectWorkspace?.state().ready&&!YPProjectWorkspace.state().busy&&!document.getElementById('templateEntryDialog').open);assert.equal(await p.evaluate(()=>getBoothSpec().cornerSide),side);assert.equal(await p.evaluate(()=>getBoothSpec().boothTemplate.id),id);assert.equal(new URL(p.url()).searchParams.has('cornerSide'),false);await ctx.close();console.log('APPLIED',id,side);}
