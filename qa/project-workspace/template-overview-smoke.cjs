...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
  for(const [folder,count,id] of [['peninsular-templates',8,'penin-orchard'],['inline-templates',5,'gallery']]){
  ...
  console.log('PASS: 13 overview use buttons; both template types open selected editable A; no template download links; saved draft cancel/recover and preserved A/new B; consumed URL; invalid ID safe; mobile CTA');
