...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
  ...
  assert.equal(await page.locator('#quickTemplateChoices [role=radio]').count(),15);assert.equal(await page.evaluate(()=>YPQuickSetupBridge.getState().draft.depth),6);
  ...
  const saved=await page.evaluate(()=>YPProjectWorkspace.capture());assert.equal(saved.active,'A');assert.equal(saved.variants.A.spec.type,'penin');assert.equal(saved.variants.A.spec.D,3);assert.equal(saved.variants.A.spec.boothTemplate.id,'penin-natural');assert.equal(await page.locator('.starter-dialog').evaluate(d=>d.open),false);
  ...
  for(const id of ['penin-aqua-curve','penin-timber-noir','penin-blue-axis','penin-orchard','penin-noir-lounge','penin-aqua-wave','penin-crimson-flow','penin-sage-ribbon','penin-blush-gallery','penin-blue-step']){
  ...
  console.log('PASS: 14 template choices, requested renders, portable files, 6x3 templates/6x6 manual, type switching, fresh A, B preserving A, mobile, no repeated starter');
