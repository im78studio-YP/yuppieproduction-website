...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
  ...
  for(const id of process.env.PENIN_SKIP_RENDER?[]:ids.filter(id=>!process.env.PENIN_RENDER_ID||id===process.env.PENIN_RENDER_ID)){const result=await page.evaluate(async id=>{
  ...
  assert.equal(await page.locator('#quickTemplateChoices [role=radio]').count(),5);assert.equal(await page.evaluate(()=>YPQuickSetupBridge.getState().draft.depth),6);
  ...
  console.log('PASS: 4 template choices, requested renders, portable files, 6x3 templates/6x6 manual, type switching, fresh A, Blue Pavilion in B preserving A, mobile, no repeated starter');
