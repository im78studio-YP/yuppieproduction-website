...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
  ...
  assert.equal(await page.locator('#quickTemplateChoices [role=radio]').count(),14);assert.equal(await page.evaluate(()=>YPQuickSetupBridge.getState().draft.depth),6);
  ...
  for(const id of ['penin-aqua-curve','penin-timber-noir','penin-blue-axis','penin-orchard','penin-noir-lounge','penin-aqua-wave','penin-crimson-flow','penin-sage-ribbon','penin-blush-gallery']){
  ...
  console.log('PASS: 13 template choices, requested renders, portable files, 6x3 templates/6x6 manual, type switching, fresh A, B preserving A, mobile, no repeated starter');
