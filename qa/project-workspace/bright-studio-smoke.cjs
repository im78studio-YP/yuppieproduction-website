...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true,args:['--enable-unsafe-swiftshader']});try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  if(!ground.getObjectByName('studio-contact-shadow')?.receiveShadow)throw Error('Ground lost contact shadows');
  ...
  return {background,ground:ground.getObjectByName('studio-satin-floor').material.uniforms.baseColor.value.getHexString(),light:light.intensity,exposure};
 });assert.deepEqual(result,{background:'e6e7eb',ground:'e6e7eb',light:.80,exposure:1.05});
