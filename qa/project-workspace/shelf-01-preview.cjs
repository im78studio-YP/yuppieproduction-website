...
(async()=>{await fs.mkdir(out,{recursive:true});const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 const items=await page.evaluate(async()=>{
  ...
  for(const item of OBJECT_CATALOG.filter(i=>i.catalogId==='imported-shelf-01'&&i.modelUrl))r.requestFurnitureTemplate(item);

  await Promise.all([...r.furnitureLoads.values(),...r.fixtureLoads.values()]);
  for(const item of OBJECT_CATALOG.filter(i=>i.catalogId==='imported-shelf-01'&&i.modelUrl))if(!r.furnitureTemplates.has(item.catalogId))throw Error('Missing model: '+item.catalogId);
  return OBJECT_CATALOG.filter(i=>i.catalogId==='imported-shelf-01').map(i=>({id:i.catalogId,name:i.name}));
 });
 for(const item of items){const data=await page.evaluate(async id=>{
  ...
  [size.x,size.y,size.z].forEach((v,i)=>{if(Math.abs(v-[1.2,2,.45][i])>.005)throw Error('Wrong shelf dimensions');});if(YPAssetParts.entries(root).length!==3)throw Error('Expected 3 editable parts');
