...
- Color and room defaults come from the chosen base. New/blank/template Wizard designs of all four booth types default to no raised floor (`raise:0`), carpet (`floor:'carpet'`), dark grey / เทาดำ (`carpet:'grey'`). Existing-work mode retains the current floor. Theme uses YPBoothTheme with graphics/plants/wood protected by default. Floor controls remain editable; the explicit restore-original button returns customization to the base snapshot.
