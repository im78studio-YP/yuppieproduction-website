...
(async()=>{
 ...
 try{
  ...
  const imageDownload=page.waitForEvent('download',{timeout:120000});
  await page.locator('#promptCopy').click();await imageDownload;
  await page.waitForFunction(()=>document.getElementById('dockPanel').getAttribute('aria-hidden')==='true');
