...
test('six unique authored arrangements',()=>{
  assert.equal(api.templates.length,6);assert.equal(new Set(api.templates.map(t=>t.id)).size,6);
  assert.equal(new Set(api.templates.map(t=>JSON.stringify(t.objects.map(o=>[o.catalogId,o.x,o.z])))).size,6);
});
for(const t of api.templates)test(t.id+' is an independent inline 6x3 snapshot with editable parts within bounds',()=>{
  ...
  boxes.forEach((b,i)=>{assert.ok(b.x0>=.1&&b.x1<=5.9&&b.z0>=.1&&b.z1<=2.9,JSON.stringify(b));if(!t.reference)assert.ok(!boxes.slice(i+1).some(other=>overlaps(b,other)),'objects overlap');assert.ok(s.objects[i].position.y>=0&&s.objects[i].position.y+s.objects[i].size.h<=2.4+.001);if(t.storage)assert.ok(!overlaps(b,{x0:4.8,x1:6,z0:0,z1:1.2}),'storage overlap');});
