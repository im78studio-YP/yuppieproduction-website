...
import '../../public/yp-web-ai/js/peninsular-oak-gallery.js';
...
test('Oak Gallery has a 6x6x3 envelope and independent editable parts',()=>{
 const initial={objects:[],sceneItemState:{},logo:'user'},before=structuredClone(initial),t=YPPeninsularOakGalleryTemplates[0],source=JSON.stringify(t),s=YPPeninsularTemplates.build(t.id,initial,catalog).spec;
 ...
 for(const o of s.objects){const a=o.rotationY*Math.PI/180,hw=(Math.abs(Math.cos(a))*o.size.w+Math.abs(Math.sin(a))*o.size.d)/2,hd=(Math.abs(Math.sin(a))*o.size.w+Math.abs(Math.cos(a))*o.size.d)/2;assert.ok(o.position.x-hw>=-.001&&o.position.x+hw<=6.001,o.id+' x');assert.ok(o.position.z-hd>=-.001&&o.position.z+hd<=6.001,o.id+' z');assert.ok(o.position.y>=0&&o.position.y+o.size.h<=3.001,o.id+' height');assert.equal(o.locked,false);}
});
