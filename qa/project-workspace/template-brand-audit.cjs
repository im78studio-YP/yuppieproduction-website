...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true,args:['--enable-unsafe-swiftshader']});try{
 ...
 assert.equal(groups.reduce((n,g)=>n+g.templates.length,0),32);
 const report=[];
 for(const group of groups)for(const t of group.templates)for(const side of group.group==='corner'?['right','left']:['']){
  ...
  const result=await page.evaluate(async ({group,id,side})=>{
   ...
   for(let i=0;i<t.objects.length;i++){
    const o=snapshot.spec.objects[i],part=t.objects[i];if(part.brandLogo||part.graphic?.startsWith('six-')||['header','poster','counter','screen','blue-info','light-rings','blue-rings','blush-beauty'].includes(part.graphic)){
   ...
   const camera={projection:'orthographic',cameraViewType:'comparison',position:{x:yellow?9:deep?-4:side==='left'?-3:9,y:yellow?6.8:black?4.5:deep?10:7,z:deep?13:11},target:{x:3,y:h*.44,z:snapshot.spec.D*.45},up:{x:0,y:1,z:0},zoom:1,near:.01,far:1000,frustum:{left:-span,right:span,top:span*.75,bottom:-span*.75}};
